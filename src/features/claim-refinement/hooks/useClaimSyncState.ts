 ...
 */
export function useClaimSyncState({
  projectId,
  claim1Text,
  enabled = true,
  ...
  // Use V2 hooks (now the only version)
  const parseClaimMutation = useParseClaimSmart();
  const generateQueriesMutation = useGenerateQueriesSmart();
  ...
  useEffect(() => {
    ...
    // If claim text differs from last synced version
    ...
    } else if (state.syncStatus === 'out-of-sync') {
      // Claim text was reverted back to last synced version – clear out-of-sync state
      setState(prev => ({
        ...prev,
        syncStatus: 'synced',
        needsSync: false,
  ...
  const syncClaim = async () => {
    ...
    try {
      ...
      const parseResult = await parseClaimMutation.mutateAsync({
        projectId,
        claimText: debouncedClaim,
      });

      // V2 format returns a simple string array
      const parsedElements = parseResult as string[];
      ...
      // Generate queries - V2 format
      const queriesResult = await generateQueriesMutation.mutateAsync({
        elements: parsedElements,
        projectId: projectId,
      });

      // V2 format returns direct array
      const queries = queriesResult as string[];

      logger.debug('[useClaimSyncState] Query generation result', {
        queriesResult,
        isArray: Array.isArray(queriesResult),
        length: Array.isArray(queriesResult) ? queriesResult.length : null,
        queries,
      });
      ...
      // Successfully synced!
      setState({
        parsedElements, // Use the normalized parsedElements
      ...
      try {
        await ProjectApiService.saveClaimSync(projectId, {
          parsedElements: parsedElements, // Now correctly maintains V2 format as strings
          searchQueries: queries,
          lastSyncedClaim: debouncedClaim,
        });
