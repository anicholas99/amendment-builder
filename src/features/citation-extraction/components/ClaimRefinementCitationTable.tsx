...
const formatLocationData = (locationData: string | { foundInAbstract?: boolean; claimLocations?: Array<{ startClaimNumber: number; endClaimNumber: number }>; patentDescriptionLocations?: Array<unknown> } | null): string => {
...
const CitationTableRow: React.FC<{
  ...
  const isCitationSaved = savedCitationIds?.has(currentMatch.id) || false;
    ...
    <Tr>
      ...
      {showActionsColumn && canShowActions && onSaveCitationMatch && (
        <Td verticalAlign="top" width="8%">
          ...
          >
            <IconButton
              icon={<Icon as={isCitationSaved ? BsBookmarkFill : FiBookmark} />}
              aria-label="Save citation"
              size="sm"
              variant="ghost"
              color={isCitationSaved ? 'blue.500' : 'text.secondary'}
              isDisabled={isCitationSaved}
              onClick={handleSaveCitation}
              _hover={{
                color: isCitationSaved ? 'blue.600' : 'text.primary',
                bg: isCitationSaved ? 'blue.50' : 'bg.hover',
              }}
            />
          </Tooltip>
        </Td>
