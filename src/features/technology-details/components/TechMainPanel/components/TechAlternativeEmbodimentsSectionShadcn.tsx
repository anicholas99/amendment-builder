import React from 'react';
import { Shuffle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { TechSectionProps } from '../types';
import { hasSectionData } from '../../../../../utils/sectionUtils';
import ContentEditableList from '../../../../../components/common/ContentEditableList';

interface TechAlternativeEmbodimentsSectionShadcnProps
  extends TechSectionProps {
  onUpdateAlternativeEmbodiments: (items: string[]) => void;
}

/**
 * Component for displaying and editing the invention alternative embodiments - shadcn/ui version
 * Maintains exact visual consistency with other shadcn/ui tech sections
 */
export const TechAlternativeEmbodimentsSectionShadcn: React.FC<TechAlternativeEmbodimentsSectionShadcnProps> =
  React.memo(
    ({ analyzedInvention, getFontSize, onUpdateAlternativeEmbodiments }) => {
      // Extract alternative embodiments data from technicalImplementation
      const technicalImplementation =
        analyzedInvention?.technicalImplementation || {};
      const alternativeEmbodiments =
        technicalImplementation?.alternative_embodiments || [];

      if (!hasSectionData(alternativeEmbodiments)) {
        return null;
      }

      return (
        <div className="px-4 py-3">
          {/* Header */}
          <div className="flex items-center gap-3 mb-3">
            <Shuffle className="w-4 h-4 text-text-secondary" />
            <span
              className={cn(
                'font-bold text-text-primary',
                getFontSize('md') === '1rem' && 'text-base',
                getFontSize('md') === '1.125rem' && 'text-lg',
                getFontSize('md') === '1.25rem' && 'text-xl'
              )}
            >
              Alternative Embodiments
            </span>
          </div>

          {/* Content */}
          <div>
            <ContentEditableList
              items={
                Array.isArray(alternativeEmbodiments)
                  ? alternativeEmbodiments
                  : []
              }
              onChange={onUpdateAlternativeEmbodiments}
              placeholder="Add an alternative embodiment..."
              fontSize="md"
              lineHeight={1.6}
            />
          </div>
        </div>
      );
    }
  );

TechAlternativeEmbodimentsSectionShadcn.displayName =
  'TechAlternativeEmbodimentsSectionShadcn';

export default TechAlternativeEmbodimentsSectionShadcn;
