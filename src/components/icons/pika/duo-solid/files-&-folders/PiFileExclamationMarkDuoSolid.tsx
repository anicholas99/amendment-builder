...
/**
 * PiFileExclamationMarkDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiFileExclamationMarkDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileExclamationMarkDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'file-exclamation-mark icon',
  ...props
}: PiFileExclamationMarkDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
