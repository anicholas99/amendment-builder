...
export class ProjectService {
  ...
  async getProsecutionOverview(
    ...
    try {
      ...
      // Use the real prosecution data service
      const { ProsecutionDataService } = await import('./prosecutionData.server-service');
      const prosecutionService = new ProsecutionDataService();
      return await prosecutionService.getProsecutionOverview(projectId, tenantId);
    } catch (error) {
      if (error instanceof ApplicationError) throw error;
