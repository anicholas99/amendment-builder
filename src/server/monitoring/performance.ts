...
export class PerformanceMonitor {
  ...
  constructor() {
    ...
    this.setThresholds('database_query', {
      warning: 100,
      error: 500,
    });
    this.setThresholds('api_request', { warning: 500, error: 2000 });
    this.setThresholds('ai_completion', {
      warning: 5000,
      error: 30000,
    });
  }
...
export function MeasurePerformance(name?: string) {
  return function (
    ...
    descriptor.value = async function (
      ...
      // Try to get performance monitor from the instance
      const monitor = this.performanceMonitor;
      if (!monitor) {
        // If no monitor available, just call the original method
 ...
 * Note: This middleware needs access to a performance monitor instance
 * It should be used with a factory function that provides the monitor
 */
