  ...
  try {
    switch (req.method) {
      case 'GET':
        ...
        return apiResponse.ok(res, { useCases: inventionData?.useCases || [] });

      case 'PUT':
        await inventionService.updateUseCases(projectId, req.body.useCases);
    ...
    logger.error('Failed to handle use cases request', {
      projectId,
      method: req.method,
      error,
    });
